// CONFIG pode vir do index.html (GitHub Pages) ou do config.js (local)
const CONFIG = window.CONFIG || { GOOGLE_CLIENT_ID: "", GOOGLE_API_KEY: "" };
const GOOGLE_CLIENT_ID = CONFIG.GOOGLE_CLIENT_ID;
const GOOGLE_API_KEY = CONFIG.GOOGLE_API_KEY;
const SCOPES = "https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/calendar openid profile email";
...
(function () {
  const saved = localStorage.getItem("agenda-theme") || "purple";
  if (saved !== "purple")
    document.documentElement.setAttribute("data-theme", saved);
  // Marca o swatch correto como ativo assim que o DOM estiver pronto
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".theme-swatch").forEach((b) => {
      const isActive = b.getAttribute("data-theme") === saved;
...
function storageKey(base) {
  if (isDemoMode)   return "demo_" + base;
...
document.getElementById("btn-mock-login").addEventListener("click", () => {
  isDemoMode = true; // ativa prefixo "demo_" antes de qualquer leitura

  const nomeSalvo = localStorage.getItem("demo_avatarName");

  let userName;
  if (nomeSalvo && nomeSalvo.trim() !== "") {
    // Sessão demo anterior encontrada — reentrar sem pedir nome
    userName = nomeSalvo;
  } else {
    // Primeira vez: pede o nome
    userName = prompt("Qual é o teu nome?", "Dev");
    if (!userName || userName.trim() === "") userName = "Dev";
  }
...
function iniciarDashboard(userName) {
  ...
  if (!localStorage.getItem(roleKey)) {
    document.getElementById("avatar-role").textContent = "Software Engineer";
  }
  ...
  try {
    ...
    if (events.length > 0) {
      agendaItems = events.map((ev) => {
        ...
        if (summary.includes("reunião") || summary.includes("reuniao") || summary.includes("meeting"))
          type = "type-reuniao";
        else if (summary.includes("estudo") || summary.includes("curso") || summary.includes("aula"))
          type = "type-estudo";
        ...
        return {
          ...
          type,
          badge: { "type-foco": "Foco", "type-estudo": "Estudo", "type-reuniao": "Reunião" }[type],
          duration: "",
          obs: ev.description || "",
          gcalId: ev.id,
        };
...
function renderAgenda() {
  ...
  if (sorted.length === 0) {
    ...
    empty.textContent = "Nenhum item na agenda. Clique em + NOVO!";
...
function buildChart(emailData, demoMode = false) {
  ...
  if (demoMode) {
    ...
    footer.textContent =
      "Conecte sua conta Google para ver o fluxo real de e-mails.";
  } else if (today >= avg) {
    badge.textContent = "📈 Dia cheio";
    badge.className = "insight-badge busy";
    footer.textContent = `Hoje recebeu ${today} emails — acima da média de ${avg}.`;
  } else {
    badge.textContent = "🌿 Dia tranquilo";
    badge.className = "insight-badge calm";
    footer.textContent = `Hoje recebeu ${today} emails — abaixo da média de ${avg}.`;
  }
...
/* ============================================================
   ...
   ============================================================ */
const compliments = [
  '"Confia no teu processo, o teu código está a ficar excelente!"',
  '"Cada bug resolvido é uma vitória. Você está crescendo!"',
  '"Você não precisa saber tudo, precisa continuar aprendendo."',
  '"Errar faz parte. Corrigir é o que te torna desenvolvedora."',
  '"Você merece estar aqui. Seu esforço prova isso todos os dias."',
  '"Uma linha de código de cada vez. Você está chegando lá!"',
];

const moodMessages = [
  compliments, // 👩‍💻
  [
    '"Pausa com café é produtividade também. ☕"',
    '"Respira. Hidrata. Depois volta ao código."',
  ],
  [
    '"Você brilha! ✨ Não deixe o impostor apagar isso."',
    '"Sua luz não some. Continue iluminando o caminho."',
  ],
];
...
const inspirations = [
  // Mulheres na Tech
  { name: "Ada Lovelace", role: "Primeira programadora", quote: '"A imaginação é a faculdade da descoberta. É ela que penetra em mundos ainda não vistos."' },
  { name: "Grace Hopper", role: "Inventora do compilador", quote: '"A frase mais perigosa na língua é: sempre fizemos assim."' },
  { name: "Katherine Johnson", role: "Matemática da NASA", quote: '"Faça o seu trabalho tão bem que eles não consigam tirar os olhos de você."' },
  { name: "Radia Perlman", role: "Criadora do Spanning Tree", quote: '"A tecnologia não deveria ser intimidante. Qualquer um pode aprender."' },
  { name: "Reshma Saujani", role: "Fundadora da Girls Who Code", quote: '"Ensine às meninas coragem, não perfeição. O mundo precisa da sua imperfeição corajosa."' },
  { name: "Hedy Lamarr", role: "Pioneira do Wi-Fi e Bluetooth", quote: '"Qualquer garota pode ser glamourosa. Só precisa ficar parada e parecer idiota."' },
  { name: "Sheryl Sandberg", role: "Ex-COO do Facebook", quote: '"No mundo de hoje, se você não está correndo riscos, você está ficando para trás."' },
  { name: "Marissa Mayer", role: "Ex-CEO do Yahoo", quote: '"Eu sempre fiz algo para o qual não estava completamente preparada. Acho que é assim que você cresce."' },
  { name: "Ginni Rometty", role: "Ex-CEO da IBM", quote: '"O crescimento e o conforto nunca coexistem."' },
  { name: "Fei-Fei Li", role: "Pioneira em IA & ImageNet", quote: '"A IA não é sobre substituir humanos — é sobre amplificar o potencial humano."' },
  { name: "Anita Borg", role: "Fundadora do Institute for Women & Tech", quote: '"Quero que as mulheres mudem a tecnologia e a tecnologia mude o mundo."' },
  { name: "Susan Wojcicki", role: "Ex-CEO do YouTube", quote: '"Em tecnologia, é possível escalar o impossível se você acreditar suficientemente."' },
  { name: "Mae Jemison", role: "1ª astronauta negra da NASA", quote: '"Nunca seja limitada pelas imaginações limitadas dos outros."' },
  { name: "Safra Catz", role: "CEO da Oracle", quote: '"Trabalhe duro, seja honesta e nunca pare de aprender."' },
  { name: "Carol Shaw", role: "1ª game designer mulher", quote: '"A programação é uma forma de arte. Você está criando algo do nada."' },
  // Ícones da Tech
  { name: "Linus Torvalds", role: "Criador do Linux", quote: '"Falar é fácil. Me mostre o código."' },
  { name: "Alan Turing", role: "Pai da computação moderna", quote: '"Às vezes são as pessoas que ninguém imagina capazes de fazer algo que fazem as coisas que ninguém imaginava."' },
  { name: "Tim Berners-Lee", role: "Criador da World Wide Web", quote: '"A web é mais uma criação social do que técnica."' },
  { name: "Donald Knuth", role: "Pai da análise de algoritmos", quote: '"A otimização prematura é a raiz de todo mal."' },
  { name: "Edsger Dijkstra", role: "Pioneiro da ciência da computação", quote: '"A simplicidade é um pré-requisito para a confiabilidade."' },
  { name: "Paul Graham", role: "Fundador da Y Combinator", quote: '"A melhor forma de ter uma boa ideia é ter muitas ideias."' },
  { name: "Linus Pauling", role: "Cientista & inovador", quote: '"A melhor maneira de ter uma boa ideia é ter muitas ideias."' },
  { name: "Vint Cerf", role: "Co-criador do protocolo TCP/IP", quote: '"A internet é um reflexo da nossa sociedade."' },
  { name: "Dennis Ritchie", role: "Criador da linguagem C", quote: '"Simplicidade é difícil de alcançar, mas vale todo o esforço."' },
  { name: "Bjarne Stroustrup", role: "Criador do C++", quote: '"Há apenas dois tipos de linguagens: as que as pessoas reclamam e as que ninguém usa."' },
  { name: "Guido van Rossum", role: "Criador do Python", quote: '"Python é uma linguagem para leitura humana, não só para máquinas."' },
  { name: "Jeff Dean", role: "Pioneiro em IA no Google", quote: '"Grandes sistemas são construídos uma camada de abstração por vez."' },
];
  ...
  try {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `Hoje é ${today}. Busque as 6 principais notícias de tecnologia desta semana (IA, lançamentos, startups, big techs, segurança, open source). Responda SOMENTE em JSON válido, sem markdown, neste formato exato:\n[\n  {"titulo": "...", "resumo": "...", "emoji": "🤖", "fonte": "Nome do veículo", "url": "https://link-da-noticia.com"},\n  ...\n]\nResumo máximo 100 caracteres. Use emojis temáticos. Inclua o URL real da notícia no campo url.`
        }]
      })
    });
    ...
    const fallback = [
      { emoji: "🤖", titulo: "IA em alta", resumo: "Modelos de linguagem continuam evoluindo rapidamente em 2025.", fonte: "TechCrunch", url: "https://techcrunch.com" },
      { emoji: "🔐", titulo: "Segurança digital", resumo: "Ataques cibernéticos batem recorde; proteção de dados em foco.", fonte: "Wired", url: "https://wired.com" },
      { emoji: "🚀", titulo: "Big Techs investem", resumo: "Google, Microsoft e Meta ampliam infraestrutura de IA.", fonte: "The Verge", url: "https://theverge.com" },
      { emoji: "💡", titulo: "Open Source cresce", resumo: "Comunidade open source bate recordes de contribuições.", fonte: "GitHub Blog", url: "https://github.blog" },
      { emoji: "📱", titulo: "Novos dispositivos", resumo: "Fabricantes apresentam novidades em hardware e wearables.", fonte: "Engadget", url: "https://engadget.com" },
      { emoji: "🧠", titulo: "Pesquisa em IA", resumo: "Avanços em modelos multimodais prometem nova era de aplicações.", fonte: "MIT Technology Review", url: "https://technologyreview.com" },
    ];
...
/* ============================================================
   ...
   ============================================================ */
const devTipsLocal = [
  { categoria: "Git", tip: "Use `git stash pop` para recuperar alterações pausadas rapidamente." },
  { categoria: "CSS", tip: "`:is()` agrupa seletores sem repetição: `:is(h1,h2,h3) a { ... }`" },
  { categoria: "JS", tip: "`structuredClone(obj)` faz deep copy nativo sem bibliotecas." },
  { categoria: "Terminal", tip: "`Ctrl+R` no terminal busca no histórico de comandos." },
  { categoria: "VS Code", tip: "`Alt+Click` cria múltiplos cursores para edição simultânea." },
  { categoria: "HTTP", tip: "Use código 422 para erros de validação de dados, não 400." },
  { categoria: "Acessibilidade", tip: "Todo input deve ter um `<label>` associado — não só placeholder." },
  { categoria: "SQL", tip: "`EXPLAIN ANALYZE` mostra o plano de execução real da query." },
  { categoria: "React", tip: "Evite useEffect para dados derivados — calcule direto no render." },
  { categoria: "Segurança", tip: "Nunca exponha stack traces em respostas de API em produção." },
];
...
(function iniciarBotaoLimparDemo() {
  ...
  function atualizarBotaoLimpar() {
    const temSessao = localStorage.getItem("demo_avatarName");
  ...
  btn.addEventListener("click", () => {
    ...
    ["kanbanTasks", "agendaItems", "avatarRole", "avatarName", "avatarImage", "studyMinutes"].forEach(
      (k) => localStorage.removeItem("demo_" + k)
    );
    atualizarBotaoLimpar();
  });
})();